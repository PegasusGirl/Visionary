...
import time
import cv2 # ❗ NEW IMPORT: Required for color space conversion (BGR <-> RGB)
...
# Custom CSS for accessibility and better UI (Keeping existing CSS)
st.markdown("""
<style>
...
def initialize_session_state():
    ...
    if 'asl_recognizer' not in st.session_state:
        # NOTE: If the ASLRecognizer relies on model files,
        # ensure those files exist in the expected path (e.g., modules/model.h5)
...
# ❗ FIX 1: Initialize session state outside of main() for guaranteed availability
...
class ASLVideoTransformer(VideoTransformerBase):
    ...
    def recv(self, frame):
        # 1. Get the frame in BGR format (default from Streamlit-WebRTC)
        img_bgr = frame.to_ndarray(format="bgr24")
        
        # 2. ❗ FIX 2: Convert BGR to RGB for most computer vision models ❗
        # This is the most likely reason for non-recognition.
        # We assume ASLRecognizer.process_frame expects RGB.
        img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        
        # 3. Process the frame (NOTE: ASLRecognizer should return RGB annotated frame)
        annotated_frame_rgb, gesture, confidence = self.asl_recognizer.process_frame(img_rgb)
        
        # 4. Convert the annotated frame back to BGR for display in the WebRTC stream
        annotated_frame_bgr = cv2.cvtColor(annotated_frame_rgb, cv2.COLOR_RGB2BGR)


        # Update gesture display periodically
        # ❗ FIX 3A: Reduced delay to 0.3s for better responsiveness ❗
        if gesture and (time.time() - self.last_gesture_update > 0.3):
            ...
            # NOTE: Removed the 'elif gesture:' tracking logic to keep the status clean
            # but keeping the update logic simple for debugging.
            
            # ❗ FIX 3B: Always update time when a valid gesture is seen/processed
            self.last_gesture_update = time.time()
        
        elif not gesture:
            # Only update status if no gesture is being detected
            if st.session_state.last_gesture is not None:
                self.status_placeholder.info("👋 Show a hand gesture...")
                st.session_state.last_gesture = None
        
        # Return the annotated frame (in BGR format)
...
def main():
    ...
    # ❗ FIX 4: CRITICAL CHECK: Ensure the ASL model is loaded before running the app
    # You MUST add an is_model_loaded() method to your ASLRecognizer class for this check to work
    # if not st.session_state.asl_recognizer.is_model_loaded():
    #     st.error("🚨 CRITICAL ERROR: ASL Recognition Model failed to load. Check ASLRecognizer.py.")
    #     return 
    ...
    with col1:
        ...
        webrtc_ctx = webrtc_streamer(
            ...
            # ❗ FIX 5: Use 'video_transformer_factory' instead of 'video_processor_factory' ❗
    ...
    # ❗ FIX 6: Force UI Rerun to display the live ASL sentence updates
    # Only rerun if the sentence has something new, or every 0.2s max.
    if st.session_state.asl_sentence:
        # Give the Streamlit server a moment to catch up and prevent a tight loop
        time.sleep(0.05) 
        st.rerun()
