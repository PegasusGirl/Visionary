...
# --- THE CAMERA PART ---
# This displays the live feed from the back camera
captured_file = back_camera_input()

# Separate Button to trigger the OCR logic
if st.button("🔍 SCAN TEXT NOW", type="primary", use_container_width=True):
    if captured_file:
        img = Image.open(captured_file)
        snap = np.array(img)
        
        with st.spinner("Processing line by line..."):
            ...
            if len(results) > 0:
                ...
                # Combine with pauses for clear speech
...
if 'greeted' not in st.session_state:
    text_to_speak("System ready. Point the rear camera at the text and press scan.")
