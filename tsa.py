...
def autoplay_text_with_js(text_to_speak):
    """
    Uses the browser's Web Speech API via JavaScript component 
    to attempt immediate audio playback.
    """
    # JavaScript code to initiate speech synthesis
    js_code = f"""
    <script>
        function speak() {{
            if ('speechSynthesis' in window) {{
                var utterance = new SpeechSynthesisUtterance('{text_to_speak}');
                // Optional: set language, volume, pitch, rate
        ...
        // Call the speak function immediately when the component loads
        speak();
    </script>
    """
    # Embed the HTML/JS component
    components.html(js_code, height=0, width=0) # Make component invisible

intial_greeting = "Welcome to Visionary. See Beyond. Hear Beyond."

autoplay_text_with_js(intial_greeting)
